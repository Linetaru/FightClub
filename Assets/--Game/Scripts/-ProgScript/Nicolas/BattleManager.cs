...
public class BattleManager : MonoBehaviour
{
    ...
    public void SpawnPlayer()
    {
        for (int i = 0; i < gameData.CharacterInfos.Count; i++)
        {
            ...
            else if (gameData.CharacterInfos[i].InputMapping.profileName == "classic")
            {
                user.Model.SetText((i + 1) + "P");
            }
            else
            {
                foreach (Character_Info characterInfo in gameData.CharacterInfos)
                {
                    if (gameData.CharacterInfos[i] != characterInfo)
                    {
                        if (gameData.CharacterInfos[i].InputMapping.profileName == characterInfo.InputMapping.profileName)
                        {
                            user.Model.SetText(gameData.CharacterInfos[i].InputMapping.profileName + "(" + (i + 1) + "P)");
                            break;
                        }
                        user.Model.SetText(gameData.CharacterInfos[i].InputMapping.profileName);
                    }
                }
